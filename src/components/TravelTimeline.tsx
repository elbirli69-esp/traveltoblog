...
export default function TravelTimeline({
  ...
  if (loading) {
    return <p className="text-sm text-slate-500">Cargando cronología…</p>;
  }
          ...
          if (ev.kind === "day-boundary") {
            return (
              <div
                key={ev.id}
                id={`tl-day-${ev.dayKey}`}
                className="sticky top-0 z-10 bg-white/95 py-2 text-sm font-bold text-teal-800 backdrop-blur"
              >
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                {ev.body && (
                  <p className="mt-0.5 line-clamp-2 text-xs text-slate-600">{ev.body}</p>
                )}
                {ev.author && (
                  <p className="mt-0.5 text-xs text-slate-400">{ev.author}</p>
                )}
              </div>
              {ev.mediaUrl && (
